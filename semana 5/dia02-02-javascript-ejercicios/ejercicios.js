...
function reemplazarPalabra(frase = "", buscar = "", reemplazar = "") {
  return frase.replace(new RegExp(buscar, "g"), reemplazar);
}
...
function limpiarArray(arr = []) {
  return arr.filter((elemento) => !!elemento);
}
...
function caracterFrecuente(texto = "") {
  ...
  for (const char of textoLimpio) {
    ...
    if (conteo[char] > maxFrecuencia) {
      maxFrecuencia = conteo[char];
      caracterFrecuente = char;
    }
  }
...
function contarOcurrencias(frase = "") {
  // Si la frase está vacía o solo tiene espacios, devolver objeto vacío
  if (frase.trim() === "") {
    return {};
  }

  // Separar la frase en palabras (eliminando espacios extras)
